...
const GoogleFitModule = (() => {
  ...
  function getToken() {
    ...
    if (!token || Date.now() > expiry - 60000) return null; // 60s safety margin
    return token;
  }
    ...
    for (const bucket of data?.bucket ?? []) {
      ...
      for (const ds of bucket.dataset ?? []) {
        for (const pt of ds.point ?? []) {
          const ms    = (+pt.endTimeNanos - +pt.startTimeNanos) / 1000000;
      ...
      if (totalMs < 60000) continue; // skip if < 1 minute (noise)

      const durH = totalMs / 3600000;
      ...
      let bedtimeStr = '—';
      if (bedtimeNano) {
        const bd = new Date(bedtimeNano / 1000000);
      ...
      let wakeStr = '—';
      if (bedtimeNano) {
        const wakeMs = bedtimeNano / 1000000 + totalMs + awakeMs;
  ...
  function _restoreCachedData() {
    try {
      ...
      if (Date.now() - ts > 24 * 3600000) return; // discard if >24h old
      let count = 0;
